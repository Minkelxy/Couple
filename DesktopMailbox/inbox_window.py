...
class InboxWindow(QMainWindow):
    ...
    def refresh(self) -> None:
        ...
        for it in items:
            ...
            if self._mode == "inbox":
                prefix = "🟡 " if not it["read"] else "✉ "
            else:
                prefix = "⏳ "
            title = it["title"]
            att = " 📎" if it["has_attachment"] else ""
    ...
    def _show_preview(self, letter_id: str) -> None:
        ...
        html = (
            f"<h3 style='margin:0'>{meta['title']}</h3>"
            f"<p style='color:#888;margin:2px 0 8px'>"
            f"{meta['author']} → {meta['recipient']} · 写于 {created}</p>"
            f"<p style='color:#e65a7a;margin:2px 0 8px'>送达时间：{deliver}</p>"
